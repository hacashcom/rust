...
fn diamond_transfer(this: &DiamondTransfer, ctx: &dyn ExecContext, sta: &mut dyn State, sto: &dyn Store) -> Ret<Vec<u8>> {
    ...
    hacd_transfer(&mut core_state, from, &to, &DiamondNumber::from(1))
}
...
fn diamond_from_to_transfer(this: &DiamondFromToTransfer, ctx: &dyn ExecContext, sta: &mut dyn State, sto: &dyn Store) -> Ret<Vec<u8>> {
    ...
    // transfer
    let mut core_state = CoreState::wrap(sta);
    hacd_transfer(&mut core_state, &from, &to, &DiamondNumber::from(dianum as u32))
}
...
fn diamond_multiple_transfer(this: &DiamondMultipleTransfer, ctx: &dyn ExecContext, sta: &mut dyn State, sto: &dyn Store) -> Ret<Vec<u8>> {
